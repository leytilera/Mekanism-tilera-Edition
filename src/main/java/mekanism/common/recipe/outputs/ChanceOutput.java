...
public class ChanceOutput extends MachineOutput<ChanceOutput> {
    ...
    public void load(NBTTagCompound nbtTags) {
        primaryOutput
            = ItemStack.loadItemStackFromNBT(nbtTags.getCompoundTag("primaryOutput"));
        secondaryOutput
            = ItemStack.loadItemStackFromNBT(nbtTags.getCompoundTag("secondaryOutput"));
    ...
    public boolean applyOutputs(
        ItemStack[] inventory, int primaryIndex, int secondaryIndex, boolean doEmit
    ) {
        if (hasPrimary()) {
            ...
            } else if (inventory[primaryIndex].isItemEqual(primaryOutput)
                       && inventory[primaryIndex].stackSize + primaryOutput.stackSize
                           <= inventory[primaryIndex].getMaxStackSize()) {
                if (doEmit) {
                    inventory[primaryIndex].stackSize += primaryOutput.stackSize;
                }
            } else {
        ...
        if (hasSecondary() && (!doEmit || checkSecondary())) {
            if (inventory[secondaryIndex] == null) {
                ...
                return true;
            } else if (inventory[secondaryIndex].isItemEqual(secondaryOutput)
                       && inventory[secondaryIndex].stackSize + primaryOutput.stackSize
                           <= inventory[secondaryIndex].getMaxStackSize()) {
                if (doEmit) {
    ...
    public ChanceOutput copy() {
        return new ChanceOutput(
            StackUtils.copy(primaryOutput),
            StackUtils.copy(secondaryOutput),
            secondaryChance
        );
    }
}
