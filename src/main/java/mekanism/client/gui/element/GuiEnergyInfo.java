...
public class GuiEnergyInfo extends GuiElement {
    ...
    public GuiEnergyInfo(IInfoHandler handler, IGuiWrapper gui, ResourceLocation def) {
        super(
            MekanismUtils.getResource(ResourceType.GUI_ELEMENT, "GuiEnergyInfo.png"),
            gui,
            def
        );
    ...
    public void mouseClicked(int xAxis, int yAxis, int button) {
        if (button == 0) {
            if (xAxis >= -21 && xAxis <= -3 && yAxis >= 142 && yAxis <= 160) {
                general.energyUnit = EnergyType.values(
                )[(general.energyUnit.ordinal() + 1) % EnergyType.values().length];
            }
        }
    }
}
