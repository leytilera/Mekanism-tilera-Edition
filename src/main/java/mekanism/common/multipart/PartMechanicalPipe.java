...
@Optional.Interface(iface = "api.hbm.fluidmk2.IFluidConnectorMK2", modid = "hbm")
public class PartMechanicalPipe extends PartTransmitter<IFluidHandler, FluidNetwork>
    implements IFluidHandler, IFluidConnectorMK2 {
        ...
        if (!world().isRemote) {
            ...
            for (ForgeDirection side : getConnections(ConnectionType.PULL)) {
                if (connectedAcceptors[side.ordinal()] != null) {
                    ...
                    if (container != null) {
                        FluidStack received
                            = container.drain(side.getOpposite(), getPullAmount(), false);

                        if (received != null && received.amount != 0) {
                            container.drain(
                                side.getOpposite(), takeFluid(received, true), true
                            );
                        }
                    }
                }
        ...
        if (getTransmitter().hasTransmitterNetwork()
            ...
            if ((last != null
                 && !(
                     ...
                     && lastWrite.getFluid() == last.getFluid()
                 ))
    ...
    private FluidStack getSaveShare() {
        if (getTransmitter().hasTransmitterNetwork()
            ...
            return PipeUtils.copy(
                getTransmitter().getTransmitterNetwork().buffer, toSave
            );
        }
    ...
    public void load(NBTTagCompound nbtTags) {
        ...
        if (nbtTags.hasKey("cacheFluid")) {
            buffer.setFluid(
                FluidStack.loadFluidStackFromNBT(nbtTags.getCompoundTag("cacheFluid"))
            );
        } else {
    ...
    public static void registerIcons(IIconRegister register) {
        pipeIcons.registerCenterIcons(
            register,
            new String[] { "MechanicalPipeBasic",
                           "MechanicalPipeAdvanced",
                           "MechanicalPipeElite",
                           "MechanicalPipeUltimate" }
        );
        pipeIcons.registerSideIcons(
            register,
            new String[] { "MechanicalPipeVerticalBasic",
                           "MechanicalPipeVerticalAdvanced",
                           "MechanicalPipeVerticalElite",
                           "MechanicalPipeVerticalUltimate",
                           "MechanicalPipeHorizontalBasic",
                           "MechanicalPipeHorizontalAdvanced",
                           "MechanicalPipeHorizontalElite",
                           "MechanicalPipeHorizontalUltimate" }
        );
    }
