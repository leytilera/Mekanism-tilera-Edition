...
public class GuiTransporterConfigTab extends GuiElement {
    ...
    public GuiTransporterConfigTab(
        IGuiWrapper gui, int y, TileEntity tile, ResourceLocation def
    ) {
        super(
            MekanismUtils.getResource(
                ResourceType.GUI_ELEMENT, "GuiTransporterConfigTab.png"
            ),
            gui,
            def
        );
    ...
    public void renderForeground(int xAxis, int yAxis) {
        ...
        if (xAxis >= -21 && xAxis <= -3 && yAxis >= yPos + 4 && yAxis <= yPos + 22) {
            displayTooltip(
                LangUtils.localize("gui.configuration.transporter"), xAxis, yAxis
            );
        }
        ...
        if (button == 0) {
            if (xAxis >= -21 && xAxis <= -3 && yAxis >= yPos + 4 && yAxis <= yPos + 22) {
                Mekanism.packetHandler.sendToServer(
                    new SimpleGuiMessage(Coord4D.get(tileEntity), 0, 51)
                );
