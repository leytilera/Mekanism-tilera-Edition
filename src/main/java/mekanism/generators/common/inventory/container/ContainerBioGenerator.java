...
public class ContainerBioGenerator extends Container {
    ...
    public ContainerBioGenerator(
        InventoryPlayer inventory, TileEntityBioGenerator tentity
    ) {
        tileEntity = tentity;
        ...
        for (slotX = 0; slotX < 3; ++slotX) {
            for (int slotY = 0; slotY < 9; ++slotY) {
                addSlotToContainer(new Slot(
                    inventory, slotY + slotX * 9 + 9, 8 + slotY * 18, 84 + slotX * 18
                ));
            }
        }
    ...
    private boolean isBiofuel(ItemStack itemStack) {
        if (FluidRegistry.isFluidRegistered("bioethanol")) {
            if (FluidContainerRegistry.getFluidForFilledItem(itemStack) != null) {
                if (FluidContainerRegistry.getFluidForFilledItem(itemStack).getFluid()
